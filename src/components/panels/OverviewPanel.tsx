...
const OverviewPanel: React.FC = () => {
  ...
  const monthlyData = Array.from({ length: 12 }, (_, i) => {
    const month = i + 1;
    const incidents = currentData.filter(item => item.Bulan === month);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
        {/* Incident Type Distribution */}
        <Card className="p-6 bg-slate-800 border-slate-700">
          <h3 className="text-lg font-semibold mb-4 text-white">Distribusi Jenis Kejadian (Top 8)</h3>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-600 bg-slate-700">
                ...
                <th className="text-left p-3 font-semibold text-slate-200">Rata-rata Durasi Respons</th>
                <th className="text-left p-3 font-semibold text-slate-200">Kasus Selesai</th>
              </tr>
            </thead>
            <tbody>
              {monthlyData.map((data, index) => {
                const monthIncidents = currentData.filter(item => item.Bulan === index + 1);
                ...
                return (
                  <tr key={index} className="border-b border-slate-600 hover:bg-slate-700/50">
                    ...
                    <td className="p-3 text-slate-300">{completed} ({completionRate}%)</td>
                  </tr>
                );
              })}
            </tbody>
